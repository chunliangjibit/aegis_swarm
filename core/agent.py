...
class Agent:
    def __init__(self, team_config, role_config, initial_pos):
        ...
        self.acceleration = pygame.math.Vector2(); self.target_pos = None; self.is_detonating = False
        
        # 【新增】Attributes for strategies
        self.self_defense_radius = 75.0 # Default value, can be overridden by GUI
        self.group_id = 0 # For strategies like split_attack

    def apply_movement_physics(self, dt, boundary_behavior, screen_width, screen_height):
        if self.acceleration.length() > 1.0: self.acceleration.scale_to_length(1.0)
    ...
    def draw(self, screen, config):
        ...
        if self.health < self.max_health:
            bar_width, bar_height = self.drone_radius * 2.5, 4
            bar_x, bar_y = self.pos.x - bar_width / 2, self.pos.y - self.drone_radius - bar_height - 5
